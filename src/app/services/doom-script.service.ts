...
export class DoomScriptService {
  ...
  private async writeBinaryToJar(mapId: number, bytecode: Uint8Array, staticFuncs: number[], tileEvents: Int32Array) {
      ...
      const reader = new ByteStream(originalBuffer);
      ...
      // Media
      this.readMarker(reader); 
      const mediaCount = reader.readUShort(); 
      reader.skip(mediaCount * 2);
      
      // Geometry
      this.readMarker(reader); reader.skip(head_numNormals * 3 * 2);
      this.readMarker(reader); reader.skip(head_numNodes * 2);
      this.readMarker(reader); reader.skip(head_numNodes);
      this.readMarker(reader); reader.skip(head_numNodes * 2); reader.skip(head_numNodes * 2); 
      this.readMarker(reader); reader.skip(head_numNodes * 2); reader.skip(head_numNodes * 2);
      this.readMarker(reader); reader.skip(head_numLeaf * 2); reader.skip(head_numLeaf * 2);
      ...
      this.readMarker(reader); reader.skip(1024); // Heightmap

      this.readMarker(reader); 
      const totalSprites = head_numNormalSprites + head_numZSprites;
      reader.skip(totalSprites * 3); 
      reader.skip(4 + totalSprites * 2); 
      reader.skip(4 + head_numZSprites); 
      reader.skip(4 + head_numZSprites); 
      ...
      this.readMarker(reader); reader.skip(12 * 2); // Static Funcs
      this.readMarker(reader); reader.skip(oldNumTileEvents * 8); // Tile Events
      this.readMarker(reader); reader.skip(oldByteCodeSize); // Bytecode
  ...
  async loadAndDisassemble(mapId: number, forceReload: boolean = false): Promise<ScriptData | null> {
    ...
    const reader = new ByteStream(buffer);
    ...
    reader.skip(46); 
    this.readMarker(reader); const mediaCount = reader.readUShort(); reader.skip(mediaCount * 2);
    this.readMarker(reader); reader.skip(head_numNormals * 3 * 2);
    this.readMarker(reader); reader.skip(head_numNodes * 2);
    this.readMarker(reader); reader.skip(head_numNodes);
    this.readMarker(reader); reader.skip(head_numNodes * 2); reader.skip(head_numNodes * 2); 
    this.readMarker(reader); reader.skip(head_numNodes * 2); reader.skip(head_numNodes * 2);
    this.readMarker(reader); reader.skip(head_numLeaf * 2); reader.skip(head_numLeaf * 2);
    this.readMarker(reader); reader.skip(head_numPolys * 2); reader.skip(head_numVerts * 5); 
    this.readMarker(reader); reader.skip(Math.floor((head_numLines + 1) / 2)); reader.skip(head_numLines * 2); reader.skip(head_numLines * 2);
    this.readMarker(reader); reader.skip(1024);
    this.readMarker(reader); 
    const totalSprites = head_numNormalSprites + head_numZSprites;
    reader.skip(totalSprites * 3); reader.skip(4 + totalSprites * 2); reader.skip(4 + head_numZSprites); reader.skip(4 + head_numZSprites);
  ...
  private readMarker(reader: ByteStream) { reader.skip(4); }
}